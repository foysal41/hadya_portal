    ...
    <div className='relative overflow-hidden h-[80vh]'>
      {/* Background Image */}
      {
        slides.map((slide, idx)=> (
          <div key={idx} className={`absolute inset-0 transition-opacity duration-[1500ms] ${active === idx? "opacity-100" : "opacity-0" }`}> 
            <Image src={slide.image} alt={slide.title} fill priority={idx===0} className={`object-cover transition-transform duration-[35000ms] ${active === idx ? "scale-[1.02]" : "scale-100"}`} ></Image>
            
            {/* Overlay */}
            <div className='absolute inset-0 bg-gradient-to-b from-black/60 via-black/40 to-black/55'></div>
