      ...
      <div className="  py-10  ">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3  gap-8 ">
          {featuredServices.map((featureService, idx) => {
             const Icon = featureService.icon;
            return (
              <div key={idx}>
               <Link href={"#"}>
                   <div
                  style={{ backgroundImage: `url("${featureService.image}")` }}
                  className="relative group h-full min-h-100 rounded-2xl overflow-hidden bg-cover bg-center bg-no-repeat hover:-translate-y-1.5 hover:shadow-xl duration-300  "
                >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span>
                        <IoArrowForwardCircleOutline
                          size={40}
                          className="group-hover:translate-x-1 duration-180 shrink-0 ease-out transition-transform"
                        />
                      </span>
                    </div>
                  </div>
